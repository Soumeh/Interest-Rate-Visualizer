...
class NonFinancialTermDepositsBySize(Base, SerializableTable):
    ...
    micro_enterprises_id: Mapped[int] = mapped_column(
        ForeignKey("enterprise_interest_rates.id"), nullable=True
    )
    micro_enterprises: Mapped[EnterpriseInterestRates] = relationship(
        EnterpriseInterestRates
    )
    small_enterprises_id: Mapped[int] = mapped_column(
        ForeignKey("enterprise_interest_rates.id"), nullable=True
    )
    small_enterprises: Mapped[EnterpriseInterestRates] = relationship(
        EnterpriseInterestRates
    )
    medium_enterprises_id: Mapped[int] = mapped_column(
        ForeignKey("enterprise_interest_rates.id"), nullable=True
    )
    medium_enterprises: Mapped[EnterpriseInterestRates] = relationship(
        EnterpriseInterestRates
    )
    large_enterprises_id: Mapped[int] = mapped_column(
        ForeignKey("enterprise_interest_rates.id"), nullable=True
    )
    large_enterprises: Mapped[EnterpriseInterestRates] = relationship(
        EnterpriseInterestRates
    )
