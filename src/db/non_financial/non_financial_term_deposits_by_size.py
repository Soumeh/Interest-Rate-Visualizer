...
class NonFinancialTermDepositsBySize(Base, SerializableTable):
    ...
    small_enterprises_id: Mapped[int] = mapped_column(ForeignKey("enterprise_interest_rates.id"), nullable=True)
    small_enterprises: Mapped[EnterpriseInterestRates] = relationship(EnterpriseInterestRates)
    medium_enterprises_id: Mapped[int] = mapped_column(ForeignKey("enterprise_interest_rates.id"), nullable=True)
    medium_enterprises: Mapped[EnterpriseInterestRates] = relationship(EnterpriseInterestRates)
    large_enterprises_id: Mapped[int] = mapped_column(ForeignKey("enterprise_interest_rates.id"), nullable=True)
    large_enterprises: Mapped[EnterpriseInterestRates] = relationship(EnterpriseInterestRates)
    ...
    async def insert(cls, session: AsyncSession, purpose: NonFinancialTermDepositPurposesBySize, year: int, month: int, row: DataFrame | Series, **extra_data) -> ResultProxy:
        ...
        medium_enterprises: ResultProxy = await EnterpriseInterestRates.insert(session, row.iloc[8:12])
        medium_enterprises_id = medium_enterprises.inserted_primary_key[0]
        large_enterprises: ResultProxy = await EnterpriseInterestRates.insert(session, row.iloc[12:16])
