...
class TotalLoansByCurrency(Base, SerializableTable):
    ...
    @classmethod
    async def insert(cls, session: AsyncSession, purpose: TotalLoanPurposesByCurrency, year: int, month: int, row: DataFrame | Series, **extra_data) -> ResultProxy:
    ...
    async def process_frame(cls, session: AsyncSession, frame: DataFrame):
        ...
        await cls._process_rows(session, date_frame, rsd_data, TotalLoanPurposesByCurrency.RSD)

        eur_data = frame.iloc[from_top:to_bottom, 5:8]
        await cls._process_rows(session, date_frame, eur_data, TotalLoanPurposesByCurrency.EUR)

        chf_data = frame.iloc[from_top:to_bottom, 8:11]
        await cls._process_rows(session, date_frame, chf_data, TotalLoanPurposesByCurrency.CHF)

        total_foreign_data = frame.iloc[from_top:to_bottom, 11:14]
        await cls._process_rows(session, date_frame, total_foreign_data, TotalLoanPurposesByCurrency.TOTAL_FOREIGN)

        total_data = frame.iloc[from_top:to_bottom, 14:17]
        await cls._process_rows(session, date_frame, total_data, TotalLoanPurposesByCurrency.TOTAL)
