...
class HouseholdLoans(Base, SerializableData):
    ...
    async def process_frame(cls, session: AsyncSession, frame: DataFrame):
        ...
        await cls._process_rows(session, date_frame, housing_data, HouseholdLoanPurposes.HOUSING)

        consumer_data = frame.iloc[from_top:to_bottom, 27:40]
        await cls._process_rows(session, date_frame, consumer_data, HouseholdLoanPurposes.CONSUMER)

        cash_data = frame.iloc[from_top:to_bottom, 40:53]
        await cls._process_rows(session, date_frame, cash_data, HouseholdLoanPurposes.CASH)

        other_data = frame.iloc[from_top:to_bottom, 53:66]
        await cls._process_rows(session, date_frame, other_data, HouseholdLoanPurposes.OTHER)
