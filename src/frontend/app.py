...
div = html.Div(id='main-container', children=[
    html.H1(children='Banka aplikacija'),

    html.Div(id='WIP-controls-container', children=[
        html.Div(id="table-type", className="dropdown-containers", children=[
            html.Label('Izaberite tip podataka:'),
            TableTypes.get_dropdown()
        ]),

        html.Div(id="table-purpse", className="dropdown-containers", children=[
            html.Label('Izaberite namenu:'),
            dcc.Dropdown({}, None, id="table-purpose-dropdown")
        ]),

        html.Div(id="time-period", className="dropdown-containers", children=[
        ...
        html.Div(className='theme-toggle-container', children=[
            html.Button(id='theme-toggle', children='Promeni temu', n_clicks=0)
        ])
    ]),
...
def update_table_purpose(table_type):
    ...
    return {selection.name: selection.value for selection in TableTypes[table_type].value.purpose}
